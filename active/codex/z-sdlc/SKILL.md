...
# SDLC
- **Product-candidate first:** SDLC exists to get a correct working candidate,
not to perfect the baseline. Do not front-load CI, provenance, ACLs,
reusable gateways, manifests, or clean-host proofs ahead of the first useful
source/runtime candidate unless that exact artifact is part of the accepted
product or the current changed risk.
- **Hard action cutoff:** after scope, authority, invariants, and proof are
known, allow no more than one diagnostic-only turn before the next material
edit, command, or acceptance check. Two process-only turns are a defect.
Delete the extra ceremony and execute the cheapest safe route.
- A wrapper, gateway, installer, manifest, capability package, or second review
is not an implementation unless the accepted product or operational contract
needs it. For test-only acceptance, prefer a disposable native mechanism and
keep production safeguards unchanged.
- Bind safety properties, not a favorite tool or transport. Tool failure changes
the route; it does not manufacture a new lifecycle phase or block the host
goal.
- A blocker packet is valid only with `GOAL_OUTCOME`, `EVIDENCE_TARGET`,
...
`CONSTRAINT_CLASS:HARD|SOFT|ASSUMED`. Missing fields or routine local
failures are `REJECT_FALSE_BLOCKER:CONTINUE_LOCAL`. Missing one proof row
blocks only that row.
- A timeout means split or fix the focused proof; it does not justify a new
infrastructure family. Browser/tool failure blocks only the affected proof
row when source/test work can continue.
- Keep proof rows separate: source, browser, disposable runtime, sealed RC,
deploy/readback, provider/publication, and production readiness are not the
same row. A missing high-risk row blocks only dependent high-risk action, not
lower-risk candidate work.


## Provenance (consolidated 2026-09-12)

Canonical body: `z-sdlc` (2026-08-13 00:38:44, 10 files, sha256 `b7276b6ce6bbf07a`).

Former names now disabled: `AGENTS/sdlc`, `CODEX/sdlc`, `NINELLC/sdlc`.
