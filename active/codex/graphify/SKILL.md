---
name: graphify
description: "Build or query a persistent knowledge graph for code, documents, or mixed project material. Use only when the user explicitly invokes /graphify, when an existing graphify-out graph should be queried, or when a bounded cross-module ownership question remains unresolved after targeted source and Git search. Do not use for routine pre-plan or pre-code discovery."
trigger: /graphify
---
...
Graphify is an advisory discovery tool, not a default coding gate. Start with
targeted current-source and Git tracing. Use Graphify only when that bounded
pass cannot answer a genuinely cross-module relationship question, or when the
user explicitly requests it. Promote every decisive graph claim back to exact
current source or runtime evidence before planning or editing behavior.
...
Canonical body: `AGENTS/graphify` (2026-07-22 03:44:59, 2 files). Mtime tie with the Codex copy, so the copy with more files won.

Former names now disabled: `CODEX/graphify`.
