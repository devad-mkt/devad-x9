...
- The accepted `v1.0` contract and its SHA are frozen under `versions/v1/` so a slower or lower-quality revision can be reverted without reconstructing old chat context.
- Read `versions/v1/LESSONS.md` before changing this skill. Add only evidence-backed lessons from completed loops; do not turn ordinary product work into loop telemetry.
- A skill revision is accepted only when its validation passes and it reduces missed requirements, review cycles, or token/time cost on a real bounded slice. Otherwise restore the prior frozen version.
...
```text
Slice: Messenger settings
Evidence worker: one verified Messenger folder and UNKNOWN/effect gates
Parent gate: hash check plus spot-check required fields and hidden states
Implementation worker: Messenger-owned local draft in named paths only
Reviewer: REQUEST_CHANGES for concrete missing controls or PASS
Implementer repair: one bounded correction
Parent: focused tests, types/format/build, authenticated browser, tiered verdict
Next: Telegram evidence only after Messenger reaches an accepted local tier
```
