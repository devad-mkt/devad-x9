...
## Tickets lane correction override

For the Tickets + Help Center adoption lane, the owner policy is narrower than
the historical routing table above: use **Astra-light** (`gpt-6-astra`, low)
for ordinary evidence, planning, implementation review and code checks;
Astra-high is prohibited; Astra-medium is allowed only for a documented,
genuinely critical major security/architecture/tenancy/persistence/notification
or RC gate. Never silently substitute a model or effort.

Persist every reviewer result to its named Markdown artifact before coding or
acceptance. A chat-only answer is `RECEIPT_NOT_PERSISTED`. Classify a local
`ChatAccount` row or `manual_user_id` discovery as
`LOCAL_OWNER_RECORD`/`LOOKUP_SNAPSHOT` only. Require a separate
`AUTHENTICATED_OWNER_BINDING` or `OPERATOR_APPROVED_BINDING` and row-level
allow/deny tests before treating it as external ownership, tenant admission,
an installer/direct link, a visitor session or a notification target.

Keep both settings ledgers in every chunk receipt: OWNER `23 parents + 25
nested = 48`; RAW SOURCE `23 parents + 27 nested = 50`. Preserve every raw
key, crosswalk and discrepancy; never delete or merge controls to make a
headline count fit.

For every Astra correction, require the durable five-field quote lock before
the next worker action: `LOCAL_RECORD` with exact model/lookup anchors;
`EXTERNAL_AUTHORITY` set to `AUTHENTICATED_OWNER_BINDING`,
`OPERATOR_APPROVED_BINDING`, or `UNKNOWN`; both owner/raw counts; and an
`ADMISSION_GATE` naming dependent rows plus allow/deny tests. A local
`ChatAccount` or `manual_user_id` snapshot never substitutes for the external
binding. If the correction is only in chat, mark `RECEIPT_NOT_PERSISTED` and
do not admit code; update plan/progress/receipt hashes in the same turn.
...
The owner-facing routing table and the frozen cost/result table are saved in
[benchmarks/2026-07-31-routing-v1/ROUTING-DECISION.md](benchmarks/2026-07-31-routing-v1/ROUTING-DECISION.md).
Per-run receipts stay in
[benchmarks/2026-07-31-routing-v1/benchmark-report.md](benchmarks/2026-07-31-routing-v1/benchmark-report.md).
...
| --- | --- | --- |
| Any bounded worker | `$z-subagent` | Packet, callback, and meter rules. |
| Coding from an accepted plan | `$z-ship` | Execute the smallest proven slice without process loops. |
| Devad/$DEVAD_ROOT repository or worktree | `$z-x9` plus its routed references | Enforce project/worktree and safety authority. |
| Adoption, legacy/reference reconciliation, vendor hunk, parity behavior | `$z-native-adopt` | Trace the native owner and use its lean evidence intake. |
| Complex adoption/parity/migration with missing owners, versions, or contracts | `$z-evidence` | Manager builds a source-bound packet before dispatch. |
...
Do not make the full `$z-evidence` workflow a global
dependency of `$z-native-adopt`: the latter contains only a lean intake for
ordinary semantic changes. Escalate only when evidence/owner binding is
actually missing. `$z-x9` applies to Devad or `$DEVAD_ROOT` work, not
unrelated repositories. `$z-ship` applies to a coding worker, not a
read-only reviewer.
