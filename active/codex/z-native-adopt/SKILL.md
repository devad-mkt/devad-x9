...
`$z-evidence` only when the missing evidence spans many
...
### From `CODEX__semantic-adoption`

### False Blocker Guard
Do not accept a conclusion-first blocker. A worker asking for a decision on
`BLOCKED`, `WAITING`, `FROZEN`, `HARD_BOUNDARY`, `RESUME_ON`, or
`OWNER_REQUIRED` must provide `GOAL_OUTCOME`, `EVIDENCE_TARGET`,
`FAILED_ROUTE`, `WHY_ROUTE_FAILURE_APPLIES_TO_ALL_ALLOWED_ROUTES`,
`CHEAPEST_WORKAROUND_TESTED`, `ACTION_NOW`,
`NEXT_MATERIAL_ACTION_OR_TRUE_OWNER_BOUNDARY`, and
`CONSTRAINT_CLASS:HARD|SOFT|ASSUMED`.
Missing fields or a failed preferred mechanism means
`REJECT_FALSE_BLOCKER:CONTINUE_LOCAL`. Preserve the semantic invariant and
change the mechanism. Product behavior proof, disposable test proof, browser
proof, deploy/readback proof, and provider/publication proof are separate rows;
missing one blocks only that row.
After the owner, gap, and invariants are known, the next meaningful event must
be adoption code, a direct executable proof, or a concrete external receipt.
Do not spend another turn producing a new plan, wrapper, gateway, installer,
manifest, or review packet for the same failure. Prefer an existing native
tool or disposable test-only route. Two process-only turns trigger immediate
execution reset, not a third analysis or renamed route.

### From `AGENTS__semantic-adoption`

## False Blocker Guard
Do not let adoption work stop because a preferred proof route failed. A message
with `BLOCKED`, `WAITING`, `FROZEN`, `HARD_BOUNDARY`, `RESUME_ON`, or
`OWNER_REQUIRED` must include `GOAL_OUTCOME`, `EVIDENCE_TARGET`,
`FAILED_ROUTE`, `WHY_ROUTE_FAILURE_APPLIES_TO_ALL_ALLOWED_ROUTES`,
`CHEAPEST_WORKAROUND_TESTED`, `ACTION_NOW`,
`NEXT_MATERIAL_ACTION_OR_TRUE_OWNER_BOUNDARY`, and
`CONSTRAINT_CLASS:HARD|SOFT|ASSUMED`. Missing fields mean
`REJECT_FALSE_BLOCKER:CONTINUE_LOCAL`.
Adoption blockers park only their behavior card. Continue other current-owner
cards, focused tests, or source work unless the missing authority is secret,
production/provider/spend, destructive, shared-collision, or owner-only.
After the owner, gap, and invariants are known, the next meaningful event must
be adoption code, a direct executable proof, or a concrete external receipt.
Do not spend another turn producing a new plan, wrapper, gateway, installer,
manifest, or review packet for the same failure. Prefer an existing native
tool or a disposable test-only route; preserve the security property rather
than the failed transport. Two process-only turns trigger immediate execution
reset, not a third analysis or renamed route.


## Provenance (consolidated 2026-09-12)

Canonical body: `z-native-adopt` (2026-08-13 00:38:44, 2 files, sha256 `d5340872182aa2d2`).

Former names now disabled: `AGENTS/semantic-adoption`, `CODEX/semantic-adoption`, `NINELLC/semantic-adoption`.
