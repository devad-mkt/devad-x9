...
- For completed per-task comparisons, use the local rollout JSONL telemetry:
  `token_usage_record.payload.usage` contains input, cached-input, cache-write,
  output, reasoning-output, and total fields; effective model and effort come
  from `turn_context.payload.model` and `.effort`.
- When a turn has repeated cumulative usage records, keep the latest record per
  `turn_id`, then sum `turn_token_usage`. Never sum cumulative
  `thread_token_usage` snapshots.
...
Record these separately for every lane:
...
- `reference cost = (uncached input * input price + cached input * cached-input price + output * output price) / 1,000,000`.

The cache hit percentage is not the cached-input price. Use the current
official rate card for the cached-input price, label the result API-equivalent,
and never present it as a Pro invoice. Keep cache writes separate; do not
assume a missing field means zero. Reasoning output is already part of output
accounting when the usage record says so; do not add it twice.

High cache does not guarantee low account usage: account-level percentages are
only a coarse cross-check and cannot attribute burn to one task.
...
## Measured Benchmark Lesson

Reference paired run, 2026-09-10. The workload and artifact checks were frozen;
only orchestration changed. These are exact local rollout measurements and
API-equivalent reference estimates, not billing records.

| Lane | Parent / children | Total tokens | Cache hit | Reference cost | Proof |
| --- | --- | ---: | ---: | ---: | --- |
| Control | Astra Low | 311,981 | 86.28% | $0.86636 | PASS |
| Delegated v2 | Astra Low + Terra Low + Luna Max | 912,206 | 80.57% | $1.87242 | Partial; Luna verifier incomplete |
| Reversed v3 | Luna Max + Astra Low | 850,044 | 86.66% | $0.69099 | PASS |

Lessons:

- The reversed lane used 62,162 fewer tokens and 6.09 percentage points more
  cache than Delegated v2, while its reference estimate was $1.18143 lower.
- More tokens do not necessarily mean higher reference cost: Luna Max plus an
  Astra child used more tokens than Control but estimated 20.2% cheaper because
  the model price mix was different.
- One paired run is descriptive only. Require at least five comparable runs,
  the same proof gate, and no incomplete child before promoting a model or
...
Canonical body: `codex-token-budget` (2026-09-10 20:57:57, 1 files, sha256 `22df1d17d8be2e3e`).

Former names now disabled: `LOOPX/z-tokens`, `LOOPX_PKG/codex-token-budget`, `NINELLC/codex-token-budget`.
