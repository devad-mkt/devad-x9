...
## 7A. Product-candidate anti-blocker ladder

Each active lane must name a direct product action before it can wait.
...
Missing browser, database, transport, deploy, or provider proof blocks only the
matching proof row. It does not block source/test work or a disposable
owner-authorized behavior proof. After two process-only updates, run
`DELIVERY_RESET` and execute the smallest safe product action.
