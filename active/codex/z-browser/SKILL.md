---
name: z-browser
description: Unified browser-control router for authenticated Chrome-profile evidence, local web-app testing, public web fetching, DOM/ARIA inspection, screenshots, console and network diagnostics, Lighthouse, and performance traces. Use whenever Codex must choose among bundled Chrome control, the Codex in-app Browser, or Chrome DevTools MCP, especially for authenticated Devad, n8n, Dokploy, site.devad.io, chat.devad.io, or post.devad.io work.
---
...
| --- | --- | --- | --- |
| Authenticated UI evidence or action | Bundled Chrome control attached to the named real profile | Same-profile CDP/DevTools only when attachment is proven | Reuse one browser binding and one target tab; capture only decisive evidence. |
...
| Real-profile Chrome bridge failure | One bounded Chrome troubleshooting pass | Ask for a supported real-profile attachment or owner-visible action | Record the concrete blocker once; do not start another local bridge. |

Explicit browser intent wins. If the owner explicitly says Chrome, Browser,
or DevTools, use that surface while preserving the authentication
...
The former entrypoints are disabled, but their detailed features and
...
Canonical body: `z-browser` (2026-08-13 00:38:44, 6 files, sha256 `2f15cc193dc43057`).

Former names now disabled: `CODEX/chrome-control`, `NINELLC/chrome-control`.
