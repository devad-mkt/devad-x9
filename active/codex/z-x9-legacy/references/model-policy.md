...
Active v3 policy: read model-policy-v3.md.

The strict matrix below is preserved for rollback/history and is inactive in
the v3 package unless the owner explicitly restores it. Model promotion now
uses task-class evidence and total verified-result tokens, including retries.

Current owner-selected canary uses Sol medium for Thinker, Terra xhigh for the
single Looper and product Workers, and Luna high for exceptional Linker
transport audit. Normal transport is deterministic code and consumes no
Linker model turn. Unsupported profiles fail setup; there is no substitution.
