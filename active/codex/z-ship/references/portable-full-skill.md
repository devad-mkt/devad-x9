...
## Canonical v2 override

`$AGENTS_HOME\skills\z-ship\SKILL.md` is the active
canonical v2 rule. This portable expansion is subordinate. If any section below
appears to permit blocker-first messaging, repeated process artifacts, or
proof-infrastructure-first execution, the v2 product-candidate-first and False
Blocker Guard rules win.
...
- Workers and Loopers do not invoke `$z-think` or `$z-sdlc`.
...
- For safe in-scope implementation and diagnostics, continue autonomously.
  Do not ask owner/thinker again for routine local edits, repeat status,
  dependency-ready tests, or normal local-readiness checks.
...
- Helper mapping rule of thumb: `LITE` -> one lightweight helper candidate check,
  `MEDIUM` -> one structured helper when direct execution is materially slower,
  `HIGH` -> no helper by default unless explicit critical review value exists.
...
- Subagents are optional and default to none. When one is justified, add a
  bounded single helper at one time. Use:
  `Sol low` for easy bounded checks, `Sol medium` for normal implementation,
  `Sol high` only for critical technical review or risk confirmation.
...
- Escalate only when the action is in a real owner/architecture/security/
  destructive/production-provider-spend/secret boundary, or when a genuine
  external authorization is absent.
- Otherwise proceed with bounded progress using the established `DONE / NEXT /
  BLOCKED / OWNER_ACTION` output contract and continue the dependency-ready slice.
- Keep `SOURCE`, `MERGED`, `DEPLOYED`, and `LIVE_EXTERNAL`/`LIVE_PROVED` claims
  strictly separate in all progress updates.
