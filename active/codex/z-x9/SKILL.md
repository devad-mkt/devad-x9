...
docs. `$z-loop-style` owns the stable default packet, direct-result, Project
Intelligence, memory, and documentation flow. `$z-loop-code` is an
experimental Controller trial for an explicit fresh disposable project only.
The retained `$z-x9-legacy` name redirects normal work to Style; neither it
nor Code is normal or production project authority.

For any path, project, worktree, reference, temporary artifact, or cleanup
under `$DEVAD_ROOT`, also read `references/cdx9-workspace-layout.md` before
...
generic `.worktrees` or a new `$DEVAD_ROOT` root path.
...
- Use native Codex worktrees only under `$DEVAD_ROOT\0-cdx-wt`. Do not create a
...
- A Worker in `$DEVAD_ROOT\0-cdx-wt\<host-id>\1-core-x9` writes to that worktree's
  relative `.devad\features`; it does not write directly into the shared
  `$DEVAD_ROOT\1-core-x9` anchor. Integration brings accepted files to the anchor.
- Temporary project material belongs under the current worktree's ignored
  ...
  `$DEVAD_ROOT\1-core-x9`, this resolves to `$DEVAD_ROOT\1-core-x9\.temp`.
- Never use `C:\tmp`, a drive root, `$DEVAD_ROOT` loose files, another project's
...
For separable app domains, keep one `DOMAIN_DELIVERY_BRANCH` per Worker and
accumulate packet-bound local chunks there until a meaningful domain milestone.
Do not rebind, rebase, open a PR, or seek review after every chunk.
`AUTOPILOT_LOCAL` lets the Worker select the next eligible owned chunk and fix
routine in-scope failures without Looper permission. After local proof, deploy
and test the exact branch/SHA as `RC_BRANCH_TEST` without merging or rebinding;
Stage1 is the default, while an unmerged production test requires separate exact
owner approval and rollback. Only `OWNER_RC_APPROVED` plus finished coding opens
one bounded current-main semantic rebind, frozen-range review, and integration
disposition. A path outside the domain envelope requires one
`CROSS_DOMAIN_CHANGE_NOTICE`; Looper asks the owner only when early claim
expansion, merge, or rebind is genuinely necessary. Otherwise defer the shared
hunk and continue the owning domain.
...
```text
Use $z-loop-style as Thinker. Also use $z-x9 as repo router.
Use $z-loop-style as Linker. Also use $z-x9 as repo router.
Use $z-loop-style as Worker. Also use $z-x9 as repo router.
```

Old `$z-x9-manager` prompts use the temporary compatibility redirect.

## Bounded Advisor Probe

Before a nontrivial plan or a final blocker, you may use one bounded, secret-safe
Reader packet containing the owner requirement, active claims, the relevant
diff/proof, the concrete failure, and exactly one question.

1. Run `opencode_doctor.py` to confirm the transport is configured.
2. Ask each configured external advisor (historically GLM 5.2 and Kimi 2.7 Code)
   **at most once**. Never loop, retry, or fan out.
3. Treat all advice as a candidate. The Worker verifies every claim against
   current source, Git, and runtime before relying on it.
4. `TOOL_UNAVAILABLE` is nonblocking and never triggers a retry loop. A missing
   advisor is not a blocker.
5. Three failed Worker attempts pause that task for Thinx review. They do not
   automatically block the wider feature.
...
Canonical body: `z-x9` (2026-08-15 14:12:40, 37 files, sha256 `80475f5a8e9cda95`).

Former names now disabled: `AGENTS/devad-x9`, `CODEX/devad-x9`, `LOOPX_PKG/devad-x9`, `NINELLC/devad-x9`.
