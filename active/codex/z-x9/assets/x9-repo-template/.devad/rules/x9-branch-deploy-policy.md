...
- `$DEVAD_ROOT\core-aio\core-x9` on `feature/post-mcp-cli-api-proof-runner-2026-06-22`
...
| State | Test | Action |
| --- | --- | --- |
| `X9_SOURCE` | path is `$DEVAD_ROOT\core-aio\core-x9`, branch is `feature/post-mcp-cli-api-proof-runner-2026-06-22`, not detached | normal X9 work |
...
If a bug is found after deploy, return to `$DEVAD_ROOT\core-aio\core-x9`, make and
...
2. Export `git diff --binary` plus `git status --short --branch` to a patch
   ...
   for analysis.
3. Restart in `$DEVAD_ROOT\core-aio\core-x9`.
