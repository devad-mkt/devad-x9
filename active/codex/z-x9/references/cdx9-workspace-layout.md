...
Read this for any work under `$DEVAD_ROOT`. It governs filesystem placement and
...
- `$DEVAD_ROOT\1-core-x9` is the common repository/integration anchor. It is not a
...
- Native Codex task worktrees belong under `$DEVAD_ROOT\0-cdx-wt`. Never rename,
...
`$DEVAD_ROOT\1-core-x9-ref\.devad-aio` is the cross-project source of truth. It should contain a
...
3. Current `$DEVAD_ROOT\1-core-x9-ref\.devad-aio` cross-project registry and decisions.
...
New top-level entries under `$DEVAD_ROOT` are limited to:
...
`$DEVAD_ROOT\.temp-old\2026-07-22\devad-aio-pre-relocation` is `OLD_UNCERTAIN` preserved material, not active authority.
...
- Short-lived generated material:
  ...
  common anchor itself, this is `$DEVAD_ROOT\1-core-x9\.temp\...`.
...
- `D:\README.md` is a drive-level pointer: project work belongs under
  `$DEVAD_ROOT`, never as a new loose drive-root file.
- `$DEVAD_ROOT\readme.txt` is the human workspace pointer.
- `$DEVAD_ROOT\1-core-x9-ref\.devad-aio\WORKSPACE.md` and `PROJECTS.md` remain the
...
- The official native Codex worktree root is `$DEVAD_ROOT\0-cdx-wt`. Do not create
