---
name: devad-x9-loop
description: Deprecated compatibility name for the quarantined X9 Loop V7 engine. Redirect normal project work to x9-loop-style; retain this package only for explicit isolated V7 canaries or forensic diagnosis.
---
# X9 Loop V7 Compatibility Redirect
Normal project work: load `$x9-loop-style` and record `COMPAT_REDIRECT:devad-x9-loop:x9-loop-style`.
`$x9-loop-code` is the fresh-project-only experimental, non-production Controller trial.
...
## Creation-only model policy
Worker creation request is `gpt-5.6-luna / max`; it never overwrites an existing or owner-changed setting. Follow-up/wake omit `model` and `thinking`; an unattested host records `MODEL_PROFILE_NOT_TOOL_ENFORCED`.
...
2. Use `linker_once.py` with the exact ACTION, acknowledgement, adapter ID, and a durable non-waking drop; waking adapters are forbidden.
3. Submit and consume `TRANSPORT_ACK`; require `CONSUMED`, `DISPATCHED`, and the old `SEND_WORK_ORDER` no longer current.
4. Only then send one signal-only `WORKER_ACTION_READY` wake carrying identities and hashes; WORKER reloads durable state and the Work Order.
...
No recurring heartbeat exists inside the Loop runtime; no Git polling, sleep loop, cache warming, sentinel, interval rounding, scheduler runtime, duplicate controller, EventStore, or model-authored pass lock is allowed. Files do not wake a Codex task. A LINKER does not stop with
